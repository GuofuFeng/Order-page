...
export interface ParsedSegment {
  lotteryType: string | undefined;
  input: string;
  parsed: ParsedInput;
}

// Export regexes for UI highlighting
export const REGEX_SIX_ZODIAC = /(?:^|[\s,，。；;.])(?:六中|六肖|6中)([马蛇龙兔虎牛鼠猪狗鸡猴羊]{6})(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_FIVE_ZODIAC = /(?:^|[\s,，。；;.])(?:五中|5中)([马蛇龙兔虎牛鼠猪狗鸡猴羊]{5})(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_FOUR_ZODIAC = /(?:^|[\s,，。；;.])(?:四中|4中)([马蛇龙兔虎牛鼠猪狗鸡猴羊]{4})(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_MULTI_ZODIAC = /(?:^|[\s,，。；;.])([马蛇龙兔虎牛鼠猪狗鸡猴羊]{2,12})(?:连肖|连)(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
...
export const REGEX_MULTI_ZODIAC_V2 = /(?:^|[\s,，。；;.])([马蛇龙兔虎牛鼠猪狗鸡猴羊]{2,12})(复试|复式)?([二三四五2345])(?:连肖|连)(?:各组)?(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_NOT_IN = /(?:^|[\s,，。；;.])([五六七八九十]{1,2}|5|6|7|8|9|10|11|12)不中[:：]?([\s\S]+?)(?:买|包|各|各号|下单)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?=$|[\s,，。；;.](?:[二三四五2345两]?(?:连肖|连|连尾|不中|中)|平|包|各|买|压|个|不中|特码|正码|合计|计|总计|共|总)|(?:[二三四五2345两]?(?:连肖|连|连尾|不中|中)|平|包|各|买|压|个|不中|特码|正码|合计|计|总计|共|总))/g;
export const REGEX_EACH = /(?:^|[\s,，。；;.])([^各买压个\n平连中包不]+)(?:各|买|压|个)(?:号)?(?:[\s\W\u4e00-\u9fa5]*?)(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_GENERIC = /(?:^|[\s,，。；;.])([马蛇龙兔虎牛鼠猪狗鸡猴羊合家野肖\d\.\s,，。；;.]*?(?:大|小|单|双|红|绿|蓝|家|野|合单|合双)+[马蛇龙兔虎牛鼠猪狗鸡猴羊合家野肖\d\.\s,，。；;.]*?)(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?=$|[\s,，。；;.](?:[二三四五2345两]?(?:连肖|连|连尾|不中|中)|平|包|各|买|压|个|不中|特码|正码|合计|计|总计|共|总)|(?:[二三四五2345两]?(?:连肖|连|连尾|不中|中)|平|包|各|买|压|个|不中|特码|正码|合计|计|总计|共|总))/g;
export const REGEX_BAO = /(?:^|[\s,，。；;.])(?:([马蛇龙兔虎牛鼠猪狗鸡猴羊]+)包|包([马蛇龙兔虎牛鼠猪狗鸡猴羊]+))(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_PING = /(?:^|[\s,，。；;.])(?:(?:平特一肖|平特肖|平特|平肖|平)([马蛇龙兔虎牛鼠猪狗鸡猴羊]+)|([马蛇龙兔虎牛鼠猪狗鸡猴羊]+)(?:平特一肖|平特肖|平特|平肖|平))(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八9十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_TAIL = /(?:^|[\s,，。；;.])(?:平特|平)(\d+)尾(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
...
export const REGEX_MULTI_TAIL_V2 = /(?:^|[\s,，。；;.])(?:【?(\d{2,10})】?)(?:各|买|压|个|包)?([二三四五2345两])连尾(?:各|买|压|个|包)?(\+?\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
export const REGEX_MULTI_TAIL_V3 = /(?:^|[\s,，。；;.])(?:([二三四五2345两])?连尾|([二三四五2345两])尾)(?:[\-\s,，。；;.]*?(\d)尾[\-\s,，。；;.]*?(\d)尾[\-\s,，。；;.]*?(\d)尾(?:[\-\s,，。；;.]*?(\d)尾)?(?:[\-\s,，。；;.]*?(\d)尾)?)(?:各|买|压|个|包)?(\+?\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)/g;
...
export const parseBetInput = (inputText: string): ParsedInput => {
  ...
  // Updated to handle only the "zodiacs first" case to avoid overlap with ADVANCED
  while ((match = REGEX_MULTI_ZODIAC.exec(textForPatterns)) !== null) {
    ...
    if (!isNaN(parsedAmt) && parsedAmt > 0) {
      ...
      if (uniqueZodiacs.length >= 2) {
        ...
        // For this pattern (鼠兔连肖100), we use all provided zodiacs as the set
  ...
  while ((match = REGEX_MULTI_ZODIAC_ADVANCED.exec(textForPatterns)) !== null) {
    ...
    groups.forEach(group => {
      ...
      const groupMatch = group.match(/^(?:[\(\)（）【】]?([马蛇龙兔虎牛鼠猪狗鸡猴羊]{2,12})[\(\)（）【】]?)(?:各|买|压|个|包)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)$/);
      if (groupMatch) {
      ...
      for (const char of zodiacNames) {
        ...
        if (zodiacIdx !== -1) {
          ...
          if (zodiacNums.length > 0) {
            const perNumAmt = Number((perZodiacAmt / zodiacNums.length).toFixed(1));
  ...
  const processMultiTailMatch = (match: RegExpExecArray, type: 'advanced' | 'v2' | 'v3') => {
    ...
    if (type === 'advanced') {
      ...
      groups.forEach(group => {
        ...
        const groupMatch = group.match(/^(?:【?(\d{2,10})】?)(?:各|买|压|个|包)?(\+?\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)$/);
        if (groupMatch) {
...
export const parseMultiLotteryInput = (inputText: string): ParsedSegment[] => {
  ...
  return parts.map(part => {
    ...
    return {
      lotteryType: parsed.recognizedLotteryType,
      input: part,
      parsed
    };
  });
