...
export const REGEX_FIVE_ELEMENTS = new RegExp(BOUNDARY + '([金木水火土])[^金木水火土\\d\\n\\r]*?(?:各|每|买|压|个)?[^金木水火土\\d\\n\\r]*?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块|斤|文|闷)?', 'g');
...
export const REGEX_SPECIAL_ATTR = new RegExp(BOUNDARY + '(红波|蓝波|绿波|大数|小数|单数|双数|红单|红双|蓝单|蓝双|绿单|绿双)(?![^\\d\\n\\r]*?(?:各|每|买|压|个))[^\\d]*?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块|斤|文|闷)?', 'g');
...
export const parseBetInput = (inputText: string): ParsedInput => {
  ...
  let preprocessedInput = inputText.replace(/(六中|六肖|6中|五中|五肖|5中|四中|四肖|4中)\s*(家|野|男|女|天|地|吉|美|凶|丑)肖?/g, (match, type, group) => {
    ...
    if (count === requiredCount) {
      ...
      const pinyinGroupName = group === '家' ? '家肖' :
                             ...
                             group === '美' ? '美肖' :
                             group === '凶' ? '凶肖' : '丑肖';
      result.errors.push(`‘${match}’是错误的，因为${pinyinGroupName}一共包含${count}个生肖，数量不等于‘${type}’本应该匹配的生肖数量(${requiredCount})`);
      return match;
    }
  });
