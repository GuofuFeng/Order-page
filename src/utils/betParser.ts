...
export const REGEX_MULTI_ZODIAC = new RegExp(BOUNDARY + '((?:[马蛇龙兔虎牛鼠猪狗鸡猴羊家野][\\s,，。；;.、/\\-*]*){2,12})(?:连肖|连)[^\\d]*?(?:各|每|买|压|个)?[^\\d]*?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块|斤|文|闷)?', 'g');
...
export const REGEX_MULTI_ZODIAC_HABIT = new RegExp(BOUNDARY + '((?:[马蛇龙兔虎牛鼠猪狗鸡猴羊家野][\\s,，。；;.、/\\-*]*){2,12})(?:([二三四五六23456两])连)\\s*(?:复([二三四五六23456两])连)\\s*(?:各|每)?(\\d+(?:\\.\\d+)?)', 'g');
...
export const REGEX_MULTI_ZODIAC_MULTI_GROUP = new RegExp(BOUNDARY + '([二三四五六23456两])连肖((?:[马蛇龙兔虎牛鼠猪狗鸡猴羊家野]{2,12}[，,、\\s]+)+[马蛇龙兔虎牛鼠猪狗鸡猴羊家野]{2,12})(?:各|每|买|压|个)?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块|斤|文|闷)?', 'g');
...
export const REGEX_BAO = new RegExp(BOUNDARY + '(?:(?:包肖|包|特码|特)[^\\d]*?([马蛇龙兔虎牛鼠猪狗鸡猴羊家野]+)|([马蛇龙兔虎牛鼠猪狗鸡猴羊家野]+)[^\\d]*?(?:包肖|包|特码|特))(?:[^\\d]*?(各|每|买|压|个))?[^\\d]*?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块|斤|文|闷)?', 'g');
export const REGEX_TE_XIAO = new RegExp(BOUNDARY + '(?:(?:特肖)[^\\d]*?([马蛇龙兔虎牛鼠猪狗鸡猴羊家野]+)|([马蛇龙兔虎牛鼠猪狗鸡猴羊家野]+)[^\\d]*?(?:特肖))(?:[^\\d]*?(各|每|买|压|个))?[^\\d]*?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块|斤|文|闷)?', 'g');
export const REGEX_PING = new RegExp(BOUNDARY + '(?:(?:平特一肖|平特肖|平特|平肖|平)[^\\d]*?([马蛇龙兔虎牛鼠猪狗鸡猴羊家野]+)|([马蛇龙兔虎牛鼠猪狗鸡猴羊家野]+)[^\\d]*?(?:平特一肖|平特肖|平特|平肖|平))(?:[^\\d]*?(各|每|买|压|个))?[^\\d]*?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块|斤|文|闷)?', 'g');
  ...
  const parsePrefix = (prefix: string): number[] => {
    ...
    if (parts.length > 0) {
      parts.forEach(part => {
        let tempPart = part;
        // Head/Tail
        const headMatches = part.match(/(\d+)头/g);
        if (headMatches) {
          headMatches.forEach(m => {
            tempPart = tempPart.replace(m, ' ');
        ...
        const tailMatches = part.match(/(\d+)尾/g);
        if (tailMatches) {
        ...
        // Numbers
        const numMatches = tempPart.match(/\d+/g);
        if (numMatches) {
        ...
        // Zodiacs and Special categories
        const zodiacMatches = part.match(/[马蛇龙兔虎牛鼠猪狗鸡猴羊家野]/g);
        if (zodiacMatches) {
          zodiacMatches.forEach(z => {
            if (z === '家') {
              domesticZodiacs.forEach(dz => currentNumbers.push(...getNumbersForZodiac(dz)));
            } else if (z === '野') {
              wildZodiacs.forEach(wz => currentNumbers.push(...getNumbersForZodiac(wz)));
            } else {
      ...
      const zodiacMatches = prefix.match(/[马蛇龙兔虎牛鼠猪狗鸡猴羊家野]/g);
      if (zodiacMatches) {
        zodiacMatches.forEach(z => {
          if (z === '家') {
            domesticZodiacs.forEach(dz => currentNumbers.push(...getNumbersForZodiac(dz)));
    ...
    // If no base numbers/zodiacs, start with all numbers if there's a filter, otherwise return empty
    const hasFilter = prefix.includes('红') || prefix.includes('绿') || prefix.includes('蓝') ||
                      prefix.includes('大') || prefix.includes('小') ||
                      prefix.replace(/合单/g, '').includes('单') ||
                      prefix.replace(/合双/g, '').includes('双') ||
                      prefix.includes('合单') || prefix.includes('合双');
  ...
  allMatches.forEach(match => {
    ...
    switch (match.type) {
      ...
      case 'MULTI_ZODIAC_ADVANCED': {
        ...
        const innerRegex = /(?:\s*)((?:[马蛇龙兔虎牛鼠猪狗鸡猴羊][\\s,，。；;.、/\-*]*){2,12})[^\d\n\r各每号]*?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:元|块|米|个|元|块|斤|文|闷)?/g;
        let m;
