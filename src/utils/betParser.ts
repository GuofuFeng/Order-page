...
// Regular Expressions
export const REGEX_SIX_ZODIAC = /(?:^|[\s,，。；;.])(?:六中|六肖|6中)([马蛇龙兔虎牛鼠猪狗鸡猴羊]{6})(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块)?/g;
export const REGEX_FIVE_ZODIAC = /(?:^|[\s,，。；;.])(?:五中|5中)([马蛇龙兔虎牛鼠猪狗鸡猴羊]{5})(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块)?/g;
export const REGEX_FOUR_ZODIAC = /(?:^|[\s,，。；;.])(?:四中|4中)([马蛇龙兔虎牛鼠猪狗鸡猴羊]{4})(?:各|买|压|个)?(\d+(?:\.\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块)?/g;
...
export const REGEX_HEAD_TAIL = new RegExp(BOUNDARY + '(\\d+)(头|尾)(?:各|买|压|个)?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块)?', 'g');
export const REGEX_EACH = new RegExp(BOUNDARY + '([^各买压个\\n平连中包不]+)(?:各|买|压|个)(?:号|码)?(\\d+(?:\\.\\d+)?|[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾两廿卅佰仟]+)(?:米|个|元|块)?', 'g');
...
export const REGEX_FLAT_NUMBER = new RegExp(BOUNDARY + '(?:(\\d+)平码(\\d+(?:\\.\\d+)?)|平码(\\d+)-(\\d+(?:\\.\\d+)?)|平码([\\d\\.\\s,，。；;.]+)各(\\d+(?:\\.\\d+)?)|平码(\\d+)各(\\d+(?:\\.\\d+)?))', 'g');
...
export const parseMultiLotteryInput = (inputText: string): ParsedSegment[] => {
  ...
  const typePattern = lotteryTypes.join('|');
